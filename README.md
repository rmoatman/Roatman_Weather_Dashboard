...
At one point in the process, I realized that a city could exist in several states.  As a result I addeded a state input box to identify specific cities.   

Once the coding was complete, I forwarded the live link to a couple of friends and invited them to "break" my code.  I suggested that they misspell city names, omit states, search for the same city twice, refresh often, clear localStorage often--and then not at all.  What I discovered from theirs and my testing was:  A drop-down box for the states would easily eliminate misspelling of state abbreviations; I needed to create the recent search buttons during the submit event instead of upon refreshing or reloading; and the API database recognizes just a few US Territory cities.
